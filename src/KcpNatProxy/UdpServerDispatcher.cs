...
namespace KcpNatProxy
{
    ...
    {
        public static UdpSocketServiceDispatcher<T> Create<T>(Socket socket, UdpServerDispatcherOptions<T> options) where T : class, IUdpService
        {
    ...
    internal sealed class UdpSocketServiceDispatcher<T> : IUdpServiceDispatcher, IDisposable where T : class, IUdpService
    {
        ...
        private readonly UdpServerDispatcherOptions<T> _options;
        ...
        public UdpSocketServiceDispatcher(Socket socket, UdpServerDispatcherOptions<T> options)
        {
            _socket = socket;
            ...
            _sendQueue = new UdpSocketSendQueue(socket, options.SendQueueCapacity);
        }
        ...
        public ValueTask SendPacketAsync(EndPoint endPoint, ReadOnlyMemory<byte> packet, CancellationToken cancellationToken)
        {
            ...
            return _sendQueue.SendAsync(endPoint, packet, cancellationToken);
        }
        ...
        public void Dispose()
        {
            ...
            _sendQueue.Dispose();
            ConcurrentDictionary<EndPoint, ServiceInfo> services = _services;
            UdpServerDispatcherOptions<T> options = _options;
            while (true)
            {
